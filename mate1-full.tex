...
\begin{abstract}
	...
	teórica profunda. Fue realizado principalmente a modo de práctica para con \LaTeX.\\
	Cualquier sugerencia, correción y/o similar, es bienvenida: \href{mailto:aheir@itba.edu.ar}{aheir@itba.edu.ar}.\\
	Se adjunta link al repositorio en GitHub del documento: 
	\href{https://github.com/anheir/mate1-full}{github.com/anheir/mate1-full} 
\end{abstract}
...
\begin{commBoxy}
	...
	\littleTitle{Tesis}
	\begin{gather*}
		\boldsymbol{\exists c \in (a,b) / f'(c) = 0}, \quad \text{\emph{se afirma al menos unas raiz de }$f'$}
	\end{gather*}
\end{commBoxy}

\subsection{Coroladio del Teorema de Rolle - C.T.R.}
\begin{commBoxy}
	\littleTitle{Hipótesis}
	\begin{itemize}
		\item $f$ continua en $\underbrace{[a,b]}_{(a,b)}$; derivable en $(a,b)$.
		\item $f'(x) = 0$ tiene exactamente $k$ soluciones en $(a,b)$
	\end{itemize}

	\littleTitle{Tesis}\par
	\begin{center}
		$f(x) = 0$ tiene \emph{como máximo} $k + 1$ soluciones en $\underbrace{[a,b]}_{(a,b)}$
	\end{center}
\end{commBoxy}

\subsection{Aplicaciones del C.T.R.}
\subsubsection{Afirmar inyectividad de una función}
Sea $f$ continua en $\underbrace{[a,b]}_{(a,b)}$, derivable en $(a,b)$, $f'(x) \neq 0 \ \forall x \in (a,b)$, entonces
\begin{center}
	{\Large $f$ es inyectiva en $\underbrace{[a,b]}_{(a,b)}$} \\
	\emph{"Si la derivada no se anula $\Rightarrow f$ es inyectiva"}
\end{center}

\subsubsection{Determinar raíces de una función}

\subsection{Teorema del Valor Medio de Lagrange - T.V.M.L.}
